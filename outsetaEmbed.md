...
export type OutsetaEmbedProps = {
    ...
    registerSkipPlanOptions: boolean
    // Profile
...
addPropertyControls(OutsetaEmbed, {
    ...
    registerSkipPlanOptions: {
        title: "Plan Options",
        type: ControlType.Enum,
        options: [true, false],
        optionTitles: ["Skip", "Show"],
        defaultValue: false,
        hidden: (props: {
            widget: Widget
            widgetMode: WidgetMode
    ...
    profileDefaultTab: {
        ...
        optionTitles: ProfileTabOptions.map(
            (tab) => ProfileTabOptionLabels[tab]
        ),
        hidden: (props: { widget: Widget }) =>
            !["profile"].includes(props.widget),
    },
    emailListUid: {
...
const AuthEmbed = ({
    ...
    const customizeProps: { planUid?: string; planFamilyUid?: string } = {}
    let subtitle = null

    if (registerPreselect === "plan") {
        customizeProps["data-plan-uid"] = registerPlanUid
        subtitle = (
            <>
    ...
    if (registerPreselect === "family") {
        customizeProps["data-plan-family-uid"] = registerFamilyUid
        subtitle = (
            <>
    ...
    if (registerSkipPlanOptions) {
        customizeProps["data-skip-plan-options"] = registerSkipPlanOptions
    }

    if (registerDiscountCode) {
        customizeProps["data-registration-defaults"] = JSON.stringify({
            Subscription: {
